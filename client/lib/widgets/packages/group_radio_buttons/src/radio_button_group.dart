...
class RadioButtonGroup extends StatefulWidget {
  ...
  /// Specifies the direction to display elements.
  final GroupedButtonsOrientation direction;
  ...
  final double radioButtonsGap;

  /// Space between Radio Button and Text
  final double radioButtonTextGap;
  ...
  const RadioButtonGroup({
    ...
    this.direction = GroupedButtonsOrientation.vertical,
    this.itemBuilder,
    this.padding = const EdgeInsets.all(0.0),
    this.margin = const EdgeInsets.all(0.0),
    this.alignment = MainAxisAlignment.start,
    this.radioButtonsGap = 0.0,
    this.radioButtonTextGap = 0.0,
  }) : super(key: key);
    ...
    for (int i = 0; i < widget.labels.length; i++) {
      ...
      if (widget.itemBuilder != null) {
        ...
        if (widget.orientation == GroupedButtonsOrientation.vertical) {
          if (widget.direction == GroupedButtonsOrientation.horizontal) {
            content.add(
              Row(
                children: <Widget>[
                  rb,
                  SizedBox(width: widget.radioButtonTextGap),
                  t,
                ],
              ),
            );
            ...
            content.add(
              Column(
                children: <Widget>[
                  rb,
                  SizedBox(height: widget.radioButtonTextGap),
                  t,
                ],
              ),
            );
          }
          content.add(SizedBox(height: widget.radioButtonsGap));
        } else {
          ...
          content.add(SizedBox(width: widget.radioButtonsGap));
        }
      }
    }
